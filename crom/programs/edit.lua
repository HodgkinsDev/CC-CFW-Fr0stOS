local tArgs = { ... }
function isPathOrSingleWord(str)
    -- Check if the string contains any slashes
...
local username = fkernel.getUsername()
local permissions = fkernel.getPermissions()  -- Assuming there's a method to get permissions


if permissions == 'User' then
    -- Get the first argument from the shell
    local args = {...}
    ...
    -- Check if the current directory begins with "disk"
    if currentDir:sub(1, 4) == "disk" then
        print("Executing command in disk: " .. (targetPath or "No target provided"))
        -- Skip all other checks and allow execution
    else
        -- Check if the target path is provided
        if not targetPath then
            term.setTextColor(colors.red)
            print("Permission Denied. You cannot edit/create files here.")
		...
		local expectedPath = "/disk"

        -- Check if the target path is a word or a path
        local pathCheck = isPathOrSingleWord(targetPath)

        -- Validate the target path
        if pathCheck == 1 then  -- It's a path
            if not (targetPath:sub(1, #expectedPath1) == expectedPath1 or targetPath:sub(1, #expectedPath) == expectedPath or targetPath:sub(1, #expectedPath2) == expectedPath2) then
                term.setTextColor(colors.red)
                print("Permission Denied. You cannot edit/create files here.")
                term.setTextColor(colors.white)
                return
            end
        elseif pathCheck == 0 then  -- It's a single word
            -- Check if the current directory begins with the expected paths
            if not (currentDir:sub(1, #expectedPath1) == expectedPath1 or currentDir:sub(1, #expectedPath2) == expectedPath2) then
                term.setTextColor(colors.red)
                print("Permission Denied. You cannot edit/create files here.")
                term.setTextColor(colors.white)
                return
